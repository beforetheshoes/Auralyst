...
struct DataExporter {
    static func exportCSV(for journal: SQLiteJournal) throws -> Data {
        ...
        for entry in dataset.entries {
            ...
            values.append(isoFormat.format(entry.timestamp))
        ...
        for medication in dataset.medications {
            ...
            values.append(isoFormat.format(medication.createdAt))
            values.append(isoFormat.format(medication.updatedAt))
            lines.append(values.joined(separator: ","))
        }
        ...
        for intake in dataset.intakes {
            ...
            values.append(isoFormat.format(intake.timestamp))
            values.append(intake.scheduledDate.map { isoFormat.format($0) } ?? "")
        ...
        for schedule in dataset.schedules {
            ...
            values.append(schedule.startDate.map { isoFormat.format($0) } ?? "")
